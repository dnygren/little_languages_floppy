...
/* For the purposes of showing how image functions are constructed, the
	...
	if (!lexicalAnalyzer(argv[1]))
		puts("error in lexical analysis");
	else 
	{
		puts("... lexical analysis completed ...");
		puts("... parsing phase beginning ...");

		if (!imlParser(&program))
			puts("error in parsing IML program");
		else
		{
			...
			if (!compileProgramFlag) result = interpretIMLProgram(program);
				else result = compileIMLProgram(program, outputFileName);
...

{
	...
	{					
		...
		strcpy(tokenValue.u.sval,tokenBuffer);
...

{
	...
	tokenType = t_real;
	tokenValue.u.dval = atof(tokenBuffer);
...

{
	...
	tokenType = t_integer;
	tokenValue.u.ival = atoi(tokenBuffer);
...

{
	...
	{
		tokenValue.u.cval = tokenBuffer[0];
		tokenType = t_special;
...

{
	...
	switch (tokenType)
	{
		case	t_keyword:
		case	t_integer:
					fprintf(file,"%d %d\n",tokenType,tokenValue.u.ival);
					break;

		case	t_identifier:
		case	t_imageFileName:
					fprintf(file,"%d %s\n",tokenType,tokenValue.u.sval);
					break;

		case	t_string:
					strcpy(temp,tokenValue.u.sval + 1);
					temp[strlen(temp) - 1] = EOS;
					fprintf(file,"%d %s\n",tokenType,temp);
					break;

		case	t_real:
					fprintf(file,"%d %lf\n",tokenType,tokenValue.u.dval);
					break;

		case	t_special:
					fprintf(file,"%d %c\n",tokenType,tokenValue.u.cval);
					break;
...

{
	...
	struct	programNode	*bldAsgStmtStruct(void);
	...
	bool	parsEndStmt(void);
	struct	programNode	*bldEndStmtStruct(void);
...

{
	...
	strcpy(fileName, tokenValue.u.sval);

	if (!readToken()) return(false);
	if (tokenValue.u.ival != k_name) return(false);

	if (!readToken()) return(false);
	if (tokenType != t_identifier) return(false);
	strcpy(imageName, tokenValue.u.sval);
						
	if (!readToken()) return(false);
	if (tokenValue.u.ival != k_index) return(false);
...

{
	...
	{
		value -> u.ival = tokenValue.u.ival;
		value -> type  = t_integer;
	}
	else if (checkToken(t_real, c_none)) 
	{
		value -> u.dval = tokenValue.u.dval;
		value -> type = t_real;
	}
	else if (checkToken(t_string, c_none)) 
	{
		strcpy(value -> u.sval,tokenValue.u.sval);
		value -> type = t_string;
	}
...

{
	...
	{
		case t_integer:
				new -> u.ival = value -> u.ival;
				break;

		case t_real:
				new -> u.dval = value -> u.dval;
				break;

		case t_string:
				new -> u.sval = allocateString(value -> u.sval);
				break;
	}
...

{
	...
	{
		case t_integer:
							p -> u.ival = value -> u.ival;
							p -> type = t_integer;
							break;

		case t_real:
							p -> u.dval = value -> u.dval;
							p -> type = t_real;
							break;

		case t_string:
							p -> u.sval = allocateString(value -> u.sval);
							p -> type = t_string;
							break;
...

{
	if (type == t_special)
	{
		if ((tokenType == t_special) && (c == tokenValue.u.cval)) return(true);
			else return(false);
	}
	else
	{
...
struct imageReferentConnectorStruct *bldImageReferent(void)
{
	int	ltype;
	int	rtype;
	int	op;
	...
	if (tokenValue.u.ival != k_and &&
		 tokenValue.u.ival != k_or &&
		 tokenValue.u.cval != c_eol) return(NULL);
	...
	while (true)
	{
		...
		if (rtype == s_connector)
			p -> rightOperand.u.connector =
				(struct imageReferentConnectorStruct *)right;

		if (rtype == s_element)
			p -> rightOperand.u.element =
		...
		if (tokenValue.u.ival != k_and && tokenValue.u.ival != k_or)
			return(NULL);
...

{
	...
	if (checkToken(t_special, c_eol))
	{
		...
		*nextTokenType = tokenType;
		nextToken -> u.cval = tokenValue.u.cval;

		return(true);
	}

	if (!isOneOf(tokenValue.u.cval, "+-()")) return(false);

	if (isOneOf(tokenValue.u.cval, "+-)"))
	{
		*expressionType = e_identifier;
	...
	while (true)
	{
		...
		switch (tokenType)
		{
			case	t_real:
									...
									new -> u.dval = tokenValue.u.dval;

									break;

			case	t_integer:
									new -> argumentType = t_integer;
									new -> u.ival = tokenValue.u.ival;

									break;

			case	t_string:
									new -> argumentType = t_string;
									new -> u.sval = allocateString(tokenValue.u.sval);

									break;

			case	t_identifier:
									new -> argumentType = t_identifier;
									new -> u.id = allocateString(tokenValue.u.sval);
...

{
	...
	struct	imageExpressionStruct	*bldImageExpressionStruct(void);
	...
	p -> sd.assignmentStmt -> imageVariableName = 
		allocateString(tokenValue.u.sval);
...

{
	...
	else
	{
		...
		if (type == e_identifier)
		{
			...
			pElement -> u.imageVariableName = allocateString(name);
		}

		if (type == e_function)
		{
			pElement -> type = e_function;
			pElement -> u.imageFunction = f;
		}
	...
	if (checkToken(t_special, c_eol))
	{
		...
		if (ltype == e_element)
			pOperand -> u.element = (struct imageExpressionElementStruct *)left;

		if (ltype == e_expression)
			pOperand -> u.expression = (struct imageExpressionStruct *)left;
	...
	if (!isOneOf(tokenValue.u.cval,"+-)")) return(NULL);
...
/* end of fix for exp24 */
	...
	while (true)
	{
		...
		if (rtype == e_element)
			pOperand -> u.element = (struct imageExpressionElementStruct *)right;

		if (rtype == e_expression)
			pOperand -> u.expression = (struct imageExpressionStruct *)right;
...
/* store the value into a value structure. */
		...
		storeValue(ip -> type, (union valueUnion *)&(ip -> u.ival), &value);
...
/* Process the left subtree */
	...
	if (isReferentConnector(&(root -> leftOperand)))
		p = processImageReferentExpression(root -> leftOperand.u.connector);
	else if (isReferentElement(&(root -> leftOperand)))
	{
		storeValue((root -> leftOperand.u.element) -> type, 
						(union valueUnion *)&((root -> leftOperand.u.element) ->  u.ival),
						&value);
		selectImage((root -> leftOperand.u.element) -> attributeName, &value, &p);
	} 

/* Process the right subtree */

	if (isReferentConnector(&(root -> rightOperand)))
		q = processImageReferentExpression(root -> rightOperand.u.connector);
	else if (isReferentElement(&(root -> rightOperand)))
	{
		storeValue((root -> rightOperand.u.element) -> type, 
						(union valueUnion *)&((root -> rightOperand.u.element) -> u.ival),
						&value);
		selectImage((root -> rightOperand.u.element) -> attributeName, &value, &q);
	} 
...

{
	...
	if (p -> type == e_identifier) return(findImage(p -> u.imageVariableName));

	if (p -> type == e_function) return(processImageFunction(p -> u.imageFunction));
...

{
	...
	if ((root -> leftOperand) -> typeOperand == e_expression)
		leftResult = processImageExpression((root -> leftOperand) -> u.expression);
	else 	if ((root -> leftOperand) -> typeOperand == e_element)
		leftResult = processImageExpressionElement((root -> leftOperand) -> u.element); 
	...
	if (root -> rightOperand != NULL)
	{
		if ((root -> rightOperand) -> typeOperand == e_expression)
			rightResult = processImageExpression((root -> rightOperand) -> u.expression);
		else if ((root -> rightOperand) -> typeOperand == e_element)
			rightResult = processImageExpressionElement((root -> rightOperand) -> u.element);
	}
...
bool writeProgramPrologue(FILE *f)
{
	...
	fprintf(f,"bool view(struct imageListStruct *l);\n");
	fprintf(f, "void error(char *msg, int action);\n");
...
bool compileAddStatement(struct programNode *p, FILE *f)
{
	...
	while (ip != NULL)
	{
		writeValueAssignment(f, ip -> type, (union valueUnion *)&(ip -> u.ival));
...
char *referentTransducer(FILE *f, struct imageReferentConnectorStruct *root)
{
	...
	if (r -> leftOperand.type == s_connector)
		p = referentTransducer(f, r -> leftOperand.u.connector);
	else
	{
		p = createTemporaryVariable(t_imageList);

		writeValueAssignment(f,(r -> leftOperand.u.element) -> type,
									(union valueUnion *)&((r -> leftOperand.u.element) -> u.ival));

		fprintf(f, "selectImage(\"%s\", &value, &%s);\n",
					(r -> leftOperand.u.element) -> attributeName,
					p);
	}
     
	/* Handle the right operand of the image referent expression tree */

	if (r -> rightOperand.u.connector != NULL)
	{
		if (r -> rightOperand.type == s_connector)
			q = referentTransducer(f, r -> rightOperand.u.connector);
		else
		{
			q = createTemporaryVariable(t_imageList);


			writeValueAssignment(f,(r -> rightOperand.u.element) -> type,
										(union valueUnion *)&((r -> rightOperand.u.element) -> u.ival));

			fprintf(f, "selectImage(\"%s\", &value, &%s);\n",
						(r -> rightOperand.u.element) -> attributeName,
						q);
		}
	}
	...
	if (r -> rightOperand.u.connector != NULL) fprintf(f, "%s);\n",q);
		else fprintf(f, "NULL);\n");
...
bool writeValueAssignment(FILE *f, int type, union valueUnion *value)
{
	switch (type)
	{
		case t_integer:		
									...
									fprintf(f, "value.u.ival = %d;\n", value -> ival);
									break;
     
		case t_real:        	
									fprintf(f, "value.type = %d;\n", t_real);
									fprintf(f, "value.u.dval = %lf;\n", value -> dval);
									break;
     
		case t_string:
		case t_identifier:
									fprintf(f, "value.type = %d;\n", t_string);
									fprintf(f, "value.u.sval = allocateString(\"%s\");\n", value -> sval);
									break;
	}
...
bool writeTypedVariable(FILE *f, int type)
{
	switch (type)
	{
		case t_integer:			fprintf(f,"u.ival");
										break;
     
		case t_real:				fprintf(f, "u.dval");
										break;
     
		case t_string:				fprintf(f, "u.sval");
										break;
	}
...
char *expressionTransducer(FILE *f, struct imageExpressionStruct *root)
{
	...
	if ((r -> leftOperand) -> typeOperand == e_expression)
		p = expressionTransducer(f, (r -> leftOperand) -> u.expression);
	else if ((r -> leftOperand) -> typeOperand == e_element)
	{
		if (((r -> leftOperand) -> u.element) -> type == e_identifier)
		{
			p = createTemporaryVariable(t_image);
			fprintf(f,"dbp = findImageInDB(\"%s\");\n",
						((r -> leftOperand)  -> u.element) -> u.imageVariableName);
		...
		{
			p = createTemporaryVariable(t_image);
			writeImageFunction(f, p, ((r -> leftOperand) -> u.element) -> u.imageFunction);
		}
	}
	...
	if (r -> rightOperand != NULL)
	{
		if ((r -> rightOperand) -> typeOperand == e_expression)
			q = expressionTransducer(f, (r -> rightOperand) -> u.expression);
		else if ((r -> rightOperand) -> typeOperand == e_element)
		{
			if (((r -> rightOperand) -> u.element) -> type == e_identifier)
			{
				q = createTemporaryVariable(t_image);
				fprintf(f,"dbp = findImageInDB(\"%s\");\n",
							((r -> rightOperand)  -> u.element) -> u.imageVariableName);
				fprintf(f,"%s = &(dbp -> image);\n",q);
			}
			else
			{
				q = createTemporaryVariable(t_image);
				writeImageFunction(f, q, ((r -> rightOperand) -> u.element) -> u.imageFunction);
			}
		}
	}
...
bool writeArgumentValue(FILE *f, struct argumentStruct *p)
{
	...
	{
		case t_integer:		fprintf(f, "arg -> u.ival = %d;\n", p -> u.ival);
									fprintf(f, "arg -> argumentType = t_integer;\n");
									break;
     
		case t_real:			fprintf(f, "arg -> u.dval = %lf;\n", p -> u.dval);
									fprintf(f, "arg -> argumentType = t_real;\n");
									break;

     	case t_identifier:
		case t_string:			
									fprintf(f, "arg -> u.sval = allocateString(\"%s\");\n", p -> u.sval);
